...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function readSidebarHint() {
      if (sidebarHintSeen === null) {
        try {
          ...
          console.warn(
            `[${PACKAGE}] localStorage unavailable — the sidebar hint may return:`,
            error,
          )
    ...
    function removeCapture() {
      const { capture } = captureState
      const root = composerElement()
      if (capture === null || root === null) return
      const range = textRangeOf(root, capture.ref)
      if (range === null) {
        publishCapture({ capture: null, present: false })
        return
      }
      if (!selectRange(root, range)) return
      ...
      window.setTimeout(() => {
        ...
        if (retry !== null) selectRange(composer, retry)
        showToast(
          '没能移除那行引用——已替你选中，按 Backspace 删除（文件仍保留在 pastes/）',
          'error',
        )
      }, 0)
    }
    ...
    function shouldOfferSidebarHint(onScreen) {
      ...
      if (betterSidebarEverAdopted) return false
    ...
    function CaptureBar() {
      ...
      const onScreen =
        capture !== null && present === true && captureBelongsToCurrentSession(capture)
      ...
      const hint = hintOpen
        ? React.createElement(
            ...
            React.createElement(
              'span',
              { className: 'dsh-auto-paste-hint-text' },
              SIDEBAR_HINT_TEXT,
            ),
            React.createElement(
              'button',
      ...
      const bar = React.createElement(
        'div',
        { className: 'dsh-auto-paste-bar' },
        React.createElement('span', { className: 'dsh-auto-paste-bar-name' }, capture.path),
        React.createElement(
          'span',
          { className: 'dsh-auto-paste-bar-meta' },
          `${capture.chars} 字符`,
        ),
        React.createElement('span', { className: 'dsh-auto-paste-bar-spacer' }),
        actions,
      )
    ...
    const SIDEBAR_ROW_TEXT =
      '未检测到 dsh-better-sidebar —— 装它后粘贴药丸会多一个「查看」，可在侧栏直接打开并编辑 pastes/ 文件；dsh 自带的文件面板只能浏览，不能改。'
    ...
    function SettingsSidebarRow() {
      ...
      if (betterSidebarEverAdopted) return null
      return React.createElement(
        'div',
        { className: 'dsh-auto-paste-row' },
        React.createElement(
          'div',
          { className: 'dsh-auto-paste-row-main' },
          React.createElement('div', { className: 'dsh-auto-paste-row-label' }, '侧栏集成'),
          React.createElement('div', { className: 'dsh-auto-paste-row-hint' }, SIDEBAR_ROW_TEXT),
        ),
      )
    }
    ...
    function adoptBetterSidebar(service) {
      ...
      console.log(
        `[${PACKAGE}] betterSidebar ${service === null ? 'gone' : `adopted (openFile=${betterSidebarCanOpen})`}`,
      )
    ...
    function apply(ctx) {
      ...
      if (!mountCaptureBar(ctx)) {
        console.warn(
          `[${PACKAGE}] capture bar unavailable (react=${React !== null}) — pastes still save, the reference just cannot be removed from the bar`,
        )
      }
